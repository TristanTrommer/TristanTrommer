      ...
      >
        {items.map(item => {
          return (
            <span
              key={item.idx}
              className="bg-opacity-80 dark:bg-opacity-75 absolute top-1/2 left-1/2 origin-[50%_50%] rounded-lg bg-white px-4 py-2 text-xs shadow-lg shadow-black/3 transition-all will-change-[transform,opacity,filter] dark:bg-gray-950"
              ref={item.tagRef}
              style={{
                filter: item.filter,
