...
export const rssJson = async () => {
  ...
  type XMLNode = Record<string, unknown> | string | null | undefined;

  const getTextContent = (node: XMLNode): string => {
    if (node && typeof node === 'object') {
      const n = node as Record<string, unknown>;
      return String(n.$text ?? n.$cdata ?? '');
    }
    return String(node ?? '');
  };
  ...
  if (channel?.category) {
    if (Array.isArray(channel.category)) {
      json.categories = channel.category.map((cat: XMLNode) =>
        getTextContent(cat)
      );
    } else {
  ...
  for (const val of items) {
    ...
    if (val?.category) {
      if (Array.isArray(val.category)) {
        itemCategories = val.category.map((cat: XMLNode) =>
          getTextContent(cat)
        );
      } else {
    ...
    const obj: IRSSItem = {
      ...
      published:
        Date.parse(
          getTextContent(val?.pubDate ?? val?.published ?? val?.created)
        ) || Date.now(),
      created:
        Date.parse(
          getTextContent(
    ...
    if (val?.enclosure) {
      if (Array.isArray(val.enclosure)) {
        obj.enclosures = val.enclosure.map((enc: Record<string, unknown>) =>
          String(enc?.url ?? '')
        );
      } else {
