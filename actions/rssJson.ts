...
export const rssJson = async () => {
  ...
  const getTextContent = (node: { $text: any; $cdata: any }) =>
    node?.$text ?? node?.$cdata ?? node ?? '';
  ...
  if (channel?.category) {
    if (Array.isArray(channel.category)) {
      json.categories = channel.category.map(
        (cat: { $text: any; $cdata: any }) => getTextContent(cat)
      );
    } else {
  ...
  for (const val of items) {
    ...
    if (val?.category) {
      if (Array.isArray(val.category)) {
        itemCategories = val.category.map((cat: { $text: any; $cdata: any }) =>
          getTextContent(cat)
        );
      } else {
    ...
    if (val?.enclosure) {
      if (Array.isArray(val.enclosure)) {
        obj.enclosures = val.enclosure.map(
          (enc: { url: any }) => enc?.url ?? ''
        );
      } else {
